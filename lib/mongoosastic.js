...
module.exports = function elasticSearchPlugin(schema, options){
  var mapping = getMapping(schema)
    ...
    , regex = options && options.regex ? options.regex : null
    , stripFields = options && options.stripFields ? options.stripFields : null

  if (useRiver)
    setUpRiver(schema);
  else
    setUpMiddlewareHooks(schema, regex, stripFields);
  ...
  */
   ...
   *
   * Modified by adamhaeger to accept:
   * @param regex - optional regex to replace html before indexing.
   * @param stripFields - array of fields to apply the regex to.
   */
  schema.statics.synchronize = function(query, regex, stripFields){
    var model = this
    ...
    stream.on('data', function(doc){
        ...
        if(regex && stripFields.length > 0 ){
            for (var i = 0; i < stripFields.length; i++) {
                var field = stripFields[i];
                doc[field] = doc[field].replace(regex, "")
            }
        }
   ...
   */
  function setUpMiddlewareHooks(schema, regex, stripFields) {
    ...
    schema.post('save', function(){
      var model = this;
        ...
        /* Added by adamhaeger. If the regex and stripFields are set, strip HTML from the fields before indexing. */
        if(regex && stripFields){
            for (var i = 0; i < stripFields.length; i++) {
                var field = stripFields[i];
                if(model[field]){
                    model[field] = model[field].replace(regex, "");
                }
            }
        }
